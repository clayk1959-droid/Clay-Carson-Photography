...
import data from "../../../data/photo-data/submit-test.json";

const photographs = data.displayOrder.map((number) => ({
  ...data.photographData[number - 1],
  src: `/galleries/submit-test/submit-test-${String(number).padStart(2, "0")}.jpg`,
}));

const hiddenPhotographs = data.photographData
  .map((photo, index) => ({
    ...photo,
    src: `/galleries/submit-test/submit-test-${String(index + 1).padStart(2, "0")}.jpg`,
  }))
